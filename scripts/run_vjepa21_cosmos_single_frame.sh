...
case "$STAGE" in
    validate) python -m experiments.vjepa21_cosmos_single_frame.validate_setup --config "$CONFIG" "$@" ;;
    data) python -m experiments.vjepa21_cosmos_single_frame.download_denseworld --config "$CONFIG" "$@" ;;
